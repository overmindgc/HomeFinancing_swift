...
class YoYoInputAlertView: UIView {
    ...
    typealias clickAlertClosure = (index: Int,contentText:String) -> Void //声明闭包，点击按钮传值
    //把申明的闭包设置成属性
...
//        self.addGestureRecognizer(tap)
        ...
        contentTextField.font = UIFont.systemFontOfSize(17)
    ...
    func clickBtnAction(sender: UIButton) {
        if (clickClosure != nil) {
            clickClosure!(index: sender.tag,contentText:contentTextField.text!)
        }
        dismiss()
    }
    //MARK:消失
